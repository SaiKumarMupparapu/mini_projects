...
public interface EnquiryRepo extends JpaRepository<Enquiry, Long> {
	@Query(value="select count(*) from Enquiry where c_id=:CId",nativeQuery = true)
	Long getTotalEnquries(Long CId);
	
	@Query(value="select count(*) from Enquiry where c_id=:CId and status=:status",nativeQuery = true)
